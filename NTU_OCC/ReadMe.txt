================================================================================
    MFC 程式庫 : NTU_OCC 專案概觀
===============================================================================

應用程式精靈已經為您建立了這個 NTU_OCC 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 NTU_OCC 應用程式的檔案。
...
NTU_OCC.h
    ...
    並宣告 CNTU_OCCApp 應用程式類別。

NTU_OCC.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CNTU_OCCApp。
...
NTU_OCCDoc.h、NTU_OCCDoc.cpp - 文件
    這些檔案包含您的 CNTU_OCCDoc 類別。編輯這些檔案，即可新增您的特
    殊文件資料，並實作檔案儲存和載入 (透過 CNTU_OCCDoc::Serialize)。

NTU_OCCView.h、NTU_OCCView.cpp - 文件的檢視
    這些檔案包含您的 CNTU_OCCView 類別。
    CNTU_OCCView 物件是用來檢視 CNTU_OCCDoc 物件。
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 NTU_OCC.pch 的先行編譯標頭 (PCH) 檔，
